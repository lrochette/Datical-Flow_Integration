...
abstract class BasePlugin extends DslDelegatingScript {
  ...
  def loadPipelines(String pluginDir, String pluginKey, String pluginName) {
		// Loop over the sub-directories in the pipelines directory
		// and evaluate pipelines if a pipeline.groovy file exists
		File pipesDir = new File(pluginDir, 'dsl/pipelines')
		pipesDir.eachDir {
			it.eachFile FileType.FILES, {
				if (it.name == 'pipeline.groovy') {
					loadGroovy(pluginDir, pluginKey, pluginName, it.absolutePath)
				}
			}
		}
  ...
  def loadDslCode(String pluginDir, String pluginKey, String pluginName) {
	...
	def loadGroovy(String pluginDir, String pluginKey, String pluginName, String dslFile) {
